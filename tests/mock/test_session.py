...
def test_connection_get_current_parameter(local_testing):
    # test no option
    conn = MockServerConnection(local_testing=local_testing)
...
@pytest.mark.parametrize("local_testing", [True, False])
def test_session_get_current_info(mock_sandbox, local_testing, monkeypatch):
    ...
    session = Session.builder.configs(options={"local_testing": local_testing}).create()
