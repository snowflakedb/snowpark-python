...
def test_basic_filter():
    ...
    # and expression
    assert origin_df.filter((col('a') >= 4) & (col('b') == 300)).collect() == [Row(400, 300, "pqr")]
...
def test_like_filter():
    ...
    assert origin_df.filter(col('a').like('%est%')).collect() == [Row("test"), Row("tttest")]
    assert origin_df.filter(col('a').like('.e.*')).collect() == []
    assert origin_df.filter(col('a').like('es__s')).collect() == [Row("es#!s"), Row("es#)s")]
    assert origin_df.filter(col('a').like('es___s')).collect() == []
    assert origin_df.filter(col('a').like('es%s')).collect() == [Row("ess"), Row("es#!s"), Row("es#)s")]
    assert origin_df.filter(col('a').like('%tt%')).collect() == [Row("tttest"), Row("tett")]
...
def test_regex_filter():
    ...
    assert origin_df.filter(col('a').regexp('.e.*')).collect() == [Row("test"), Row("tett")]
    assert origin_df.filter(col('a').regexp('.*s')).collect() == [Row("ess"), Row("es#%s")]
