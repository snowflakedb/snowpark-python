...
postgres_host = POSTGRES_CONNECTION_PARAMETERS["host"]
postgres_port = POSTGRES_CONNECTION_PARAMETERS["port"]
...
expected_schema = StructType(
    [
        ...
        StructField(
            "DATE_COL", TimestampType(timezone=TimestampTimeZone.NTZ), nullable=True
        ),
        StructField(
            "TIMESTAMP_COL",
...
expected_sql_no_partition = [
    "SELECT SNOWPARK_JDBC_SELECT_SQL_ALIAS.* FROM (SELECT ID, NUMBER_COL, BINARY_FLOAT_COL, BINARY_DOUBLE_COL, VARCHAR2_COL, CHAR_COL, CLOB_COL, NCHAR_COL, NVARCHAR2_COL, NCLOB_COL, DATE_COL, TIMESTAMP_COL, TIMESTAMP_TZ_COL, TIMESTAMP_LTZ_COL, RAW_COL, GUID_COL FROM ALL_TYPE_TABLE_JDBC) SNOWPARK_JDBC_SELECT_SQL_ALIAS"
]

expected_sql_partition_column = [
    "SELECT SNOWPARK_JDBC_SELECT_SQL_ALIAS.* FROM (SELECT ID, NUMBER_COL, BINARY_FLOAT_COL, BINARY_DOUBLE_COL, VARCHAR2_COL, CHAR_COL, CLOB_COL, NCHAR_COL, NVARCHAR2_COL, NCLOB_COL, DATE_COL, TIMESTAMP_COL, TIMESTAMP_TZ_COL, TIMESTAMP_LTZ_COL, RAW_COL, GUID_COL FROM ALL_TYPE_TABLE_JDBC) SNOWPARK_JDBC_SELECT_SQL_ALIAS WHERE ID < '3' OR ID is null",
    "SELECT SNOWPARK_JDBC_SELECT_SQL_ALIAS.* FROM (SELECT ID, NUMBER_COL, BINARY_FLOAT_COL, BINARY_DOUBLE_COL, VARCHAR2_COL, CHAR_COL, CLOB_COL, NCHAR_COL, NVARCHAR2_COL, NCLOB_COL, DATE_COL, TIMESTAMP_COL, TIMESTAMP_TZ_COL, TIMESTAMP_LTZ_COL, RAW_COL, GUID_COL FROM ALL_TYPE_TABLE_JDBC) SNOWPARK_JDBC_SELECT_SQL_ALIAS WHERE ID >= '3' AND ID < '6'",
    "SELECT SNOWPARK_JDBC_SELECT_SQL_ALIAS.* FROM (SELECT ID, NUMBER_COL, BINARY_FLOAT_COL, BINARY_DOUBLE_COL, VARCHAR2_COL, CHAR_COL, CLOB_COL, NCHAR_COL, NVARCHAR2_COL, NCLOB_COL, DATE_COL, TIMESTAMP_COL, TIMESTAMP_TZ_COL, TIMESTAMP_LTZ_COL, RAW_COL, GUID_COL FROM ALL_TYPE_TABLE_JDBC) SNOWPARK_JDBC_SELECT_SQL_ALIAS WHERE ID >= '6'",
]
...
expected_data = [
    Row(
        ...
        DATE_COL=datetime.datetime(2024, 3, 20, 0, 0),
    ...
    Row(
        ...
        DATE_COL=datetime.datetime(2024, 9, 27, 0, 0),
    ...
    Row(
        ...
        DATE_COL=datetime.datetime(2024, 6, 16, 0, 0),
    ...
    Row(
        ...
        DATE_COL=datetime.datetime(2024, 9, 5, 0, 0),
    ...
    Row(
        ...
        DATE_COL=datetime.datetime(2024, 11, 28, 0, 0),
    ...
    Row(
        ...
        DATE_COL=datetime.datetime(2024, 3, 19, 0, 0),
    ...
    Row(
        ...
        DATE_COL=datetime.datetime(2024, 5, 2, 0, 0),
    ...
    Row(
        ...
        DATE_COL=datetime.datetime(2024, 8, 3, 0, 0),
    ...
    Row(
        ...
        DATE_COL=datetime.datetime(2024, 12, 10, 0, 0),
...
postgres_expected_data = [
    Row(
        BIGINT_COL=-6645531000000000123,
        BIGSERIAL_COL=1,
        BIT_COL="0000",
        BIT_VARYING_COL="10100101",
        BOOLEAN_COL="f",
        BOX_COL="(13.68,50.87),(-14.5,-36.82)",
        BYTEA_COL="\\x62333566656231643034386236316163366333",
        CHAR_COL="6U0hXrbbRm",
        VARCHAR_COL="almMRlCPh3onp9celUXb",
        CIDR_COL="63.9.184.0/24",
        CIRCLE_COL="<(-29.47,-12.75),49.48>",
        DATE_COL=datetime.date(2004, 5, 9),
        DOUBLE_PRECISION_COL=1858055000.0,
        INET_COL="5.188.71.132",
        INTEGER_COL=1865101000,
        INTERVAL_COL="4 years 10 mons 15 days 18:38:52",
        JSON_COL='{"key": 123}',
        JSONB_COL='{"jsonb_key": 83}',
        LINE_COL="{-2.18,8.69,3.09}",
        LSEG_COL="[(5.12,-83.91),(41.89,62.49)]",
        MACADDR_COL="e8:16:cd:a9:9f:e6",
        MACADDR8_COL="e3:da:aa:fc:fb:51:86:f5",
        NUMERIC_COL=Decimal("113414.83"),
        PATH_COL="((43.79,36.77),(-64.49,-34.68))",
        PG_LSN_COL="85D2538C/FFC30C2E",
        PG_SNAPSHOT_COL="96:207:",
        POINT_COL="(61.05,18.47)",
        POLYGON_COL="((48.52,53.43),(89.46,69.09),(89.54,10.13))",
        REAL_COL=64374.96,
        SMALLINT_COL=-10428,
        SMALLSERIAL_COL=1,
        SERIAL_COL=1,
        TEXT_COL="OaVsansivU5I1BLQdUbRaYyzbYDmK6e",
        TIME_COL=datetime.time(19, 28, 51),
        TIMESTAMP_COL=datetime.datetime(2002, 2, 16, 2, 6),
        TIMESTAMPTZ_COL=datetime.datetime(2020, 1, 24, 12, 0),
        TSQUERY_COL="'word3' & 'word1'",
        TSVECTOR_COL="'lex1':1 'lex2':2 'lex4':3",
        TXID_SNAPSHOT_COL="10:20:10,14,15",
        UUID_COL="69ad9235-6c5e-4f95-b179-9730ed771aa8",
        XML_COL='"<root><element>34</element></root>"',
    ),
    Row(
        BIGINT_COL=-8065880000000000456,
        BIGSERIAL_COL=2,
        BIT_COL="1010",
        BIT_VARYING_COL="11100010",
        BOOLEAN_COL="f",
        BOX_COL="(29.78,3.39),(26.92,-5.57)",
        BYTEA_COL="\\xc3ac343937376464663539653033646136633165",
        CHAR_COL="yhrAXej1DO",
        VARCHAR_COL="CnwnTp8SLJKTSeQAi8oW",
        CIDR_COL="205.202.89.0/24",
        CIRCLE_COL="<(13.94,18.73),4.52>",
        DATE_COL=datetime.date(2016, 7, 9),
        DOUBLE_PRECISION_COL=9561914000.0,
        INET_COL="136.74.101.171",
        INTEGER_COL=-2129554000,
        INTERVAL_COL="4 years 4 mons 18:37:09",
        JSON_COL='{"key": 123}',
        JSONB_COL='{"jsonb_key": 79}',
        LINE_COL="{-9.8,-0.07,0.36}",
        LSEG_COL="[(21.72,55.15),(-25.05,94.64)]",
        MACADDR_COL="00:28:95:21:77:65",
        MACADDR8_COL="58:3a:c1:f7:a1:f4:d3:31",
        NUMERIC_COL=Decimal("-636436.38"),
        PATH_COL="((-0.45,-24.74),(1.41,-82.18))",
        PG_LSN_COL="D4997CEE/F6A7D891",
        PG_SNAPSHOT_COL="214:931:",
        POINT_COL="(-3.91,82.16)",
        POLYGON_COL="((21.36,22.34),(-4.94,90.88),(30.78,18.14),(25.06,60.1),(-98.04,74.43))",
        REAL_COL=65115.97,
        SMALLINT_COL=32062,
        SMALLSERIAL_COL=2,
        SERIAL_COL=2,
        TEXT_COL="M6Z9jw QXfvErDkIj3xYeAg0IVCTrTnWx5hS4kSu",
        TIME_COL=datetime.time(14, 41, 19),
        TIMESTAMP_COL=datetime.datetime(2024, 12, 25, 7, 53, 11),
        TIMESTAMPTZ_COL=datetime.datetime(2000, 8, 20, 14, 26, 46),
        TSQUERY_COL="'word2' & 'word3'",
        TSVECTOR_COL="'lex1':1 'lex2':2 'lex3':3",
        TXID_SNAPSHOT_COL="701:924:720,800",
        UUID_COL="721f1281-9f97-4b0a-b41a-bd789ad318ea",
        XML_COL='"<root><element>29</element></root>"',
    ),
    Row(
        BIGINT_COL=9083626000000000789,
        BIGSERIAL_COL=3,
        BIT_COL="1100",
        BIT_VARYING_COL="11000110",
        BOOLEAN_COL="f",
        BOX_COL="(9.1,86.79),(-46.22,-5.63)",
        BYTEA_COL="\\x52643734356239373534313038336235366239",
        CHAR_COL="wTTtppE3ND",
        VARCHAR_COL="mKb7K1PTNCoAdRGQHb2C",
        CIDR_COL="145.149.241.0/24",
        CIRCLE_COL="<(-91.43,-25.5),2.19>",
        DATE_COL=datetime.date(2019, 6, 9),
        DOUBLE_PRECISION_COL=-3365294000.0,
        INET_COL="11.106.208.59",
        INTEGER_COL=-1889577000,
        INTERVAL_COL="1 year 10 mons 23 days 21:40:12",
        JSON_COL='{"key": null}',
        JSONB_COL='{"jsonb_key": 39}',
        LINE_COL="{6.62,9.28,1.6}",
        LSEG_COL="[(-13.37,-20.05),(-96.04,-48.7)]",
        MACADDR_COL="4a:a0:71:cc:05:06",
        MACADDR8_COL="dc:c8:be:f4:0c:62:69:9c",
        NUMERIC_COL=Decimal("261715.46"),
        PATH_COL="((-66.03,-52.76),(-53.48,82.21),(-99.03,7.58),(-63.51,97.87))",
        PG_LSN_COL="3F5DA3B6/515C58E5",
        PG_SNAPSHOT_COL="86:763:",
        POINT_COL="(-74.55,7.85)",
        POLYGON_COL="((-6.65,69.89),(30.98,-66.15),(-96.04,-9.29))",
        REAL_COL=-81603.21,
        SMALLINT_COL=-32235,
        SMALLSERIAL_COL=3,
        SERIAL_COL=3,
        TEXT_COL="SWPw6w6zINbqWooVf wC90ixy0Huv9tAIbaNkTrKolY48E",
        TIME_COL=datetime.time(7, 20, 38),
        TIMESTAMP_COL=datetime.datetime(2017, 1, 28, 6, 29, 17),
        TIMESTAMPTZ_COL=datetime.datetime(2009, 10, 1, 1, 16, 17),
        TSQUERY_COL="'word3' & 'word2'",
        TSVECTOR_COL="'lex1':2 'lex3':1 'lex4':3",
        TXID_SNAPSHOT_COL="691:960:712,892",
        UUID_COL="d0164090-8196-42f0-88f6-979a4039214d",
        XML_COL='"<root><element>50</element></root>"',
    ),
    Row(
        BIGINT_COL=8517274000000000123,
        BIGSERIAL_COL=4,
        BIT_COL="1010",
        BIT_VARYING_COL="10100111",
        BOOLEAN_COL="f",
        BOX_COL="(-26.08,-37.14),(-51.59,-94.61)",
        BYTEA_COL="\\x6e303435376530333132646439383364633839",
        CHAR_COL="8jCXO6fI1J",
        VARCHAR_COL="2UENUrNFXEeMArKotkwP",
        CIDR_COL="222.181.247.0/24",
        CIRCLE_COL="<(67.48,8.88),20.47>",
        DATE_COL=datetime.date(2012, 8, 5),
        DOUBLE_PRECISION_COL=-3428823000.0,
        INET_COL="173.30.26.53",
        INTEGER_COL=-495502045,
        INTERVAL_COL="5 mons 25 days 21:24:37",
        JSON_COL='{"key": null}',
        JSONB_COL='{"jsonb_key": 54}',
        LINE_COL="{7.34,8.92,-4.74}",
        LSEG_COL="[(-33.09,-91.78),(-46.63,-75.51)]",
        MACADDR_COL="aa:26:f0:fb:6e:3a",
        MACADDR8_COL="e0:1a:46:50:7d:b0:9f:0b",
        NUMERIC_COL=Decimal("-786274.55"),
        PATH_COL="((-68.79,-68.06),(-86.31,79.21),(-84.42,-97),(-80.85,-31.05),(84.56,12.15))",
        PG_LSN_COL="457D6A04/B65BEF56",
        PG_SNAPSHOT_COL="343:454:",
        POINT_COL="(-27.87,-96.35)",
        POLYGON_COL="((-13.3,49.21),(88.92,57.41),(-11.51,71.87),(61.62,72.2),(57.03,-84.98),(73.6,-11.46))",
        REAL_COL=62962.21,
        SMALLINT_COL=32364,
        SMALLSERIAL_COL=4,
        SERIAL_COL=4,
        TEXT_COL="0wMQO5LsnVsn80kMaI 9bWKZnJJcZh VWV2cAG14Q",
        TIME_COL=datetime.time(9, 52, 2),
        TIMESTAMP_COL=datetime.datetime(2015, 5, 26, 19, 39, 51),
        TIMESTAMPTZ_COL=datetime.datetime(2015, 11, 14, 8, 36, 6),
        TSQUERY_COL="'word3' & 'word4'",
        TSVECTOR_COL="'lex2':2 'lex3':1 'lex4':3",
        TXID_SNAPSHOT_COL="667:828:699,799",
        UUID_COL="45befd8f-992d-4df6-93f0-b879792d16fe",
        XML_COL='"<root><element>95</element></root>"',
    ),
    Row(
        BIGINT_COL=-5569903000000000456,
        BIGSERIAL_COL=5,
        BIT_COL="1100",
        BIT_VARYING_COL="11110110",
        BOOLEAN_COL="f",
        BOX_COL="(49.79,76.36),(-92.6,65.41)",
        BYTEA_COL="\\xc2b9623833626266363731666635653234343163",
        CHAR_COL="wOR7ebsM5d",
        VARCHAR_COL="eludr6sfjh0rI1LLq73q",
        CIDR_COL="170.29.227.0/24",
        CIRCLE_COL="<(-10.95,-27.76),47.88>",
        DATE_COL=datetime.date(2018, 4, 29),
        DOUBLE_PRECISION_COL=-6298310000.0,
        INET_COL="75.135.255.85",
        INTEGER_COL=310008582,
        INTERVAL_COL="5 years 8 mons 5 days 00:18:31",
        JSON_COL='{"key": "value1"}',
        JSONB_COL='{"jsonb_key": 37}',
        LINE_COL="{-1.37,5.98,-6.45}",
        LSEG_COL="[(54.17,21.95),(47.55,82.92)]",
        MACADDR_COL="41:a3:38:5a:23:13",
        MACADDR8_COL="4c:9e:ea:48:2b:8d:5b:39",
        NUMERIC_COL=Decimal("525117.11"),
        PATH_COL="((-55.68,-40.24),(7.22,-65.82),(88.94,-5.42))",
        PG_LSN_COL="BCE30B1/AE3C727",
        PG_SNAPSHOT_COL="147:739:",
        POINT_COL="(-86.34,-55.89)",
        POLYGON_COL="((38.51,61.05),(84.17,17.95),(73.11,-73.06),(-66.17,-78.89),(21.63,-71.6),(-40.84,-70.27))",
        REAL_COL=25392.21,
        SMALLINT_COL=-30587,
        SMALLSERIAL_COL=5,
        SERIAL_COL=5,
        TEXT_COL="KeS36QdtwKnI9vFZ1GakjEBnNLlRpTrLEyzB",
        TIME_COL=datetime.time(19, 35, 6),
        TIMESTAMP_COL=datetime.datetime(2005, 6, 25, 22, 2, 58),
        TIMESTAMPTZ_COL=datetime.datetime(2017, 5, 24, 3, 3, 4),
        TSQUERY_COL="'word2' & 'word3'",
        TSVECTOR_COL="'lex2':2 'lex3':1 'lex4':3",
        TXID_SNAPSHOT_COL="853:868:854,855",
        UUID_COL="960b86a9-a8dd-4634-bc1f-956ae6589726",
        XML_COL='"<root><element>47</element></root>"',
    ),
    Row(
        BIGINT_COL=None,
        BIGSERIAL_COL=6,
        BIT_COL=None,
        BIT_VARYING_COL=None,
        BOOLEAN_COL=None,
        BOX_COL=None,
        BYTEA_COL=None,
        CHAR_COL=None,
