...
def test_to_sql_interval_types():
    ...
    assert (
        ...
        == "INTERVAL '+5 00:00:00.000000' DAY TO SECOND"
    )
    assert (
        to_sql(
        ...
        == "INTERVAL '+2 03:04:05.000006' DAY TO SECOND"
    )
    assert (
        to_sql(td(seconds=30, microseconds=500000), DayTimeIntervalType())
        == "INTERVAL '+0 00:00:30.500000' DAY TO SECOND"
    )
    ...
    assert (
        ...
        == "INTERVAL '-3 00:00:00.000000' DAY TO SECOND"
    )

    # YearMonthInterval: positive values
    assert to_sql(14, YearMonthIntervalType()) == "INTERVAL '+1-02' YEAR TO MONTH"
    assert to_sql(6, YearMonthIntervalType()) == "INTERVAL '+0-06' YEAR TO MONTH"

    # YearMonthInterval: zero and negative
    assert to_sql(0, YearMonthIntervalType()) == "INTERVAL '+0-00' YEAR TO MONTH"
    assert to_sql(-18, YearMonthIntervalType()) == "INTERVAL '-1-06' YEAR TO MONTH"
