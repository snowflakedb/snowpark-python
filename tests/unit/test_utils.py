...
def test_get_line_numbers():
    def assert_query_line_interval(
        actual: QueryLineInterval, expected: QueryLineInterval
    ) -> None:
        assert actual.start == expected.start
        assert actual.end == expected.end
        assert actual.uuid == expected.uuid
        assert actual.query_idx == expected.query_idx
...
-- child-uuid-2"""
    ...
    child_uuids = [("child-uuid-1", 0), ("child-uuid-2", 1)]
    parent_uuid = "parent-uuid"
    parent_query_idx = 0

    result = get_line_numbers(sql_query, child_uuids, parent_uuid, parent_query_idx)

    expected = [
        QueryLineInterval(0, 1, "parent-uuid", 0),
        QueryLineInterval(2, 2, "child-uuid-1", 0),
        QueryLineInterval(3, 3, "parent-uuid", 0),
        QueryLineInterval(4, 4, "child-uuid-2", 1),
    ]

    assert len(result) == len(expected)
    for i, a in enumerate(result):
        assert_query_line_interval(a, expected[i])
...
WHERE col1 > 0"""
    ...
    expected = [QueryLineInterval(0, 2, "parent-uuid", 0)]

    assert len(result) == 1
    assert_query_line_interval(result[0], expected[0])
...
-- uuid-2
    ...
    expected = [
        QueryLineInterval(0, 0, "parent-uuid", 0),
        QueryLineInterval(1, 7, "uuid-1", 0),
        QueryLineInterval(8, 8, "parent-uuid", 0),
        QueryLineInterval(9, 15, "uuid-2", 1),
        QueryLineInterval(16, 16, "parent-uuid", 0),
    ]
...
def test_get_plan_from_line_numbers():
    ...
    intervals = [QueryLineInterval(0, 5, "plan-uuid-123", 0)]
    simple_query = """SELECT
    id,
    name
FROM users
...
ORDER BY name"""
    ...
    result = get_plan_from_line_numbers(plan, 3, 0)
    assert result.uuid == "plan-uuid-123"

    # Test finding plan with child plans
    child_intervals = [QueryLineInterval(0, 2, "child-uuid-456", 0)]
    child_query = """SELECT *
FROM orders
WHERE status = 'pending'"""
    ...
    parent_intervals = [
        QueryLineInterval(0, 2, "parent-uuid-789", 0),
        QueryLineInterval(3, 5, "child-uuid-456", 0),
        QueryLineInterval(6, 8, "parent-uuid-789", 0),
    ]
...
GROUP BY u.name"""
    ...
    result = get_plan_from_line_numbers(parent_plan, 3, 0)
    assert result.uuid == "child-uuid-456"

    # Test error when query_idx is out of range
    intervals = [QueryLineInterval(0, 0, "error-plan-uuid-111", 0)]
    error_query = "SELECT COUNT(*) FROM products"

    with mock.patch("uuid.uuid4", return_value=mock.Mock(hex="error-plan-uuid-111")):
        error_mock_query = mock.Mock()
        error_mock_query.sql = error_query
        error_mock_query.query_line_intervals = intervals

        error_plan = mock.Mock(spec=SnowflakePlan)
        error_plan.uuid = "error-plan-uuid-111"
        error_plan.children_plan_nodes = []
        error_plan.queries = [error_mock_query]

    with pytest.raises(ValueError, match="Query idx 5 is out of range"):
        get_plan_from_line_numbers(error_plan, 0, 5)

    # Test error when line number doesn't fall in any interval
    intervals = [QueryLineInterval(0, 3, "interval-plan-uuid-222", 0)]
...
FROM products GROUP BY category"""
    ...
    with pytest.raises(
        ...
        get_plan_from_line_numbers(interval_plan, 10, 0)

    # Test finding plan where children have descendants
    grandchild_intervals = [QueryLineInterval(0, 1, "grandchild-uuid-999", 0)]
...
FROM inventory"""
    ...
    child_with_desc_intervals = [
        QueryLineInterval(0, 1, "child-desc-uuid-888", 0),
        QueryLineInterval(2, 3, "grandchild-uuid-999", 0),  # Points to grandchild
        QueryLineInterval(4, 5, "child-desc-uuid-888", 0),
    ]
...
FROM inventory
    ...
    grandparent_intervals = [
        QueryLineInterval(0, 2, "grandparent-uuid-777", 0),
        QueryLineInterval(3, 8, "child-desc-uuid-888", 0),
        QueryLineInterval(9, 10, "grandparent-uuid-777", 0),
    ]
...
ORDER BY c.name"""
    ...
    result = get_plan_from_line_numbers(grandparent_plan, 6, 0)
    assert result.uuid == "grandchild-uuid-999"

    result = get_plan_from_line_numbers(grandparent_plan, 4, 0)
    assert result.uuid == "child-desc-uuid-888"

    result = get_plan_from_line_numbers(grandparent_plan, 1, 0)
