...
def test_add_snowpark_package():
    version = metadata.distribution(SNOWPARK_PACKAGE_NAME).version
    valid_packages = {SNOWPARK_PACKAGE_NAME: [version]}
    result_dict = {}
    add_snowpark_package(result_dict, valid_packages)
    assert result_dict == {SNOWPARK_PACKAGE_NAME: f"{SNOWPARK_PACKAGE_NAME}=={version}"}
...
def test_add_snowpark_package_if_missing():
    ...
    valid_packages = {SNOWPARK_PACKAGE_NAME: [version]}
    result_dict = {}
    with patch("importlib.metadata.distribution") as mock_get_distribution:
        mock_get_distribution.side_effect = metadata.PackageNotFoundError(
            "Package not found"
        )
