...
#
"""Catalog integration tests with ``context._is_snowpark_connect_compatible_mode`` False (REST / Root backend).

Keep this file separate from ``test_catalog_sql_mode.py`` so removing one backend path
...
def _catalog_rest_backend_mode(session):
    mp = pytest.MonkeyPatch()
    mp.setattr(context, "_is_snowpark_connect_compatible_mode", False)
...
def test_list_db_rest_mode(session, temp_db1, temp_db2):
...
def test_list_schema_rest_mode(session, temp_db1, temp_schema1, temp_schema2):
    catalog: Catalog = session.catalog
    assert (
...
def test_get_db_schema_rest_mode(session):
    catalog: Catalog = session.catalog
...
def test_get_database_missing_raises_core_not_found_rest_mode(session):
    catalog: Catalog = session.catalog
...
@pytest.mark.udf
def test_get_procedure_rest_mode(session, temp_db1, temp_schema1, temp_procedure1):
    catalog: Catalog = session.catalog
...
@pytest.mark.udf
def test_get_user_defined_function_rest_mode(
    session, temp_db1, temp_schema1, temp_udf1
):
    catalog: Catalog = session.catalog
...
def test_exists_db_schema_rest_mode(session, temp_db1, temp_schema1):
...
def test_exists_table_view_rest_mode(
    session, temp_db1, temp_schema1, temp_table1, temp_view1
):
    catalog = session.catalog
...
def test_drop_rest_mode(session, use_object):
    catalog = session.catalog
