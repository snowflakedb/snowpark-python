...
def test_day_time_interval_type_dataframe(session):
    schema = StructType(
        [
            StructField("ID", LongType(), nullable=False),
            StructField("DT_INTERVAL", DayTimeIntervalType(), nullable=False),
        ]
    )

    df = session.sql(
        """
        SELECT 1 as id, INTERVAL '1 12:30:45' DAY TO SECOND as dt_interval
        UNION ALL
        SELECT 2 as id, INTERVAL '2 08:15:30' DAY TO SECOND as dt_interval
        UNION ALL
        SELECT 3 as id, INTERVAL '0 06:45:15' DAY TO SECOND as dt_interval
    """
    )
    ...
    assert result[0]["DT_INTERVAL"] == timedelta(days=1, seconds=45045)
    assert result[1]["DT_INTERVAL"] == timedelta(days=2, seconds=29730)
    assert result[2]["DT_INTERVAL"] == timedelta(seconds=24315)
    ...
    assert result[0]["DT_INTERVAL"] == timedelta(days=-2, seconds=41355)
    assert result[1]["DT_INTERVAL"] == timedelta(days=-3, seconds=56670)
    assert result[2]["DT_INTERVAL"] == timedelta(days=-1, seconds=62085)
    ...
    schema = StructType(
        [
            StructField("interval_col_day_0_0", DayTimeIntervalType(0, 0)),
            StructField("interval_col_hour_1_1", DayTimeIntervalType(1, 1)),
            StructField("interval_col_minute_2_2", DayTimeIntervalType(2, 2)),
            StructField("interval_col_second_3_3", DayTimeIntervalType(3, 3)),
            StructField("interval_col_day_hour_0_1", DayTimeIntervalType(0, 1)),
            StructField("interval_col_day_minute_0_2", DayTimeIntervalType(0, 2)),
            StructField("interval_col_day_second_0_3", DayTimeIntervalType(0, 3)),
            StructField("interval_col_hour_minute_1_2", DayTimeIntervalType(1, 2)),
            StructField("interval_col_hour_second_1_3", DayTimeIntervalType(1, 3)),
            StructField("interval_col_minute_second_2_3", DayTimeIntervalType(2, 3)),
        ]
    )
    df = session.create_dataframe(test_cases, schema=schema)
    ...
    assert test_result[0][0] == timedelta(days=2)
    assert test_result[0][1] == timedelta(hours=1)
    assert test_result[0][2] == timedelta(minutes=30)
    assert test_result[0][3] == timedelta(seconds=45)
    assert test_result[0][4] == timedelta(days=1, hours=12)
    assert test_result[0][5] == timedelta(days=1, hours=12, minutes=30)
    assert test_result[0][6] == timedelta(days=1, hours=12, minutes=30, seconds=45)
    assert test_result[0][7] == timedelta(seconds=45000)
    assert test_result[0][8] == timedelta(seconds=45045)
    assert test_result[0][9] == timedelta(seconds=1845)

    assert test_result[1][0] == timedelta(days=-3)
    assert test_result[1][1] == timedelta(hours=-2)
    assert test_result[1][2] == timedelta(minutes=-45)
    assert test_result[1][3] == timedelta(seconds=-30)
    assert test_result[1][4] == timedelta(days=-2, hours=-6)
    assert test_result[1][5] == timedelta(days=-2, hours=-6, minutes=-15)
    assert test_result[1][6] == timedelta(days=-2, hours=-6, minutes=-15, seconds=-30)
    assert test_result[1][7] == timedelta(days=-1, seconds=18900)
    assert test_result[1][8] == timedelta(days=-1, seconds=18885)
    assert test_result[1][9] == timedelta(days=-1, seconds=80970)

    assert isinstance(df.schema.fields[0].datatype, DayTimeIntervalType)
    assert (
        df.schema.fields[0].datatype.start_field == 0
        and df.schema.fields[0].datatype.end_field == 0
    )  # DAY
    assert (
        df.schema.fields[1].datatype.start_field == 1
        and df.schema.fields[1].datatype.end_field == 1
    )  # HOUR
    assert (
        df.schema.fields[2].datatype.start_field == 2
        and df.schema.fields[2].datatype.end_field == 2
    )  # MINUTE
    assert (
        df.schema.fields[3].datatype.start_field == 3
        and df.schema.fields[3].datatype.end_field == 3
    )  # SECOND
    assert (
        df.schema.fields[4].datatype.start_field == 0
        and df.schema.fields[4].datatype.end_field == 1
    )  # DAY TO HOUR
    assert (
        df.schema.fields[5].datatype.start_field == 0
        and df.schema.fields[5].datatype.end_field == 2
    )  # DAY TO MINUTE
    assert (
        df.schema.fields[6].datatype.start_field == 0
        and df.schema.fields[6].datatype.end_field == 3
    )  # DAY TO SECOND
    assert (
        df.schema.fields[7].datatype.start_field == 1
        and df.schema.fields[7].datatype.end_field == 2
    )  # HOUR TO MINUTE
    assert (
        df.schema.fields[8].datatype.start_field == 1
        and df.schema.fields[8].datatype.end_field == 3
    )  # HOUR TO SECOND
    assert (
        df.schema.fields[9].datatype.start_field == 2
        and df.schema.fields[9].datatype.end_field == 3
    )  # MINUTE TO SECOND
