...
def test_to_read_csv_from_local(session, resources_path):
...
def test_to_read_csv_from_local_directory(session, tmpdir):
...
def test_to_read_json_from_local(session, resources_path):

    test_files = TestFiles(resources_path)

    df = session.read.json(Utils.escape_path(test_files.test_file_json))
    Utils.check_answer(
        df, [Row('{\n  "color": "Red",\n  "fruit": "Apple",\n  "size": "Large"\n}')]
    )
    Utils.check_answer(
    ...
    # assert user cannot input a schema to read json
    with pytest.raises(ValueError):
        session.read.schema(user_schema).json(
            Utils.escape_path(test_files.test_file_json)
        )

    # user can input customized formatTypeOptions
    df2 = session.read.option("FILE_EXTENSION", "json").json(
        Utils.escape_path(test_files.test_file_json)
    )
    Utils.check_answer(
        df2,
...
def test_to_read_avro_from_local(session, resources_path):

    test_files = TestFiles(resources_path)

    df = session.read.option("INFER_SCHEMA", False).avro(
    ...
    # assert user cannot input a schema to read avro
    with pytest.raises(ValueError):
        session.read.schema(user_schema).avro(
            Utils.escape_path(test_files.test_file_avro)
        )

    # user can input customized formatTypeOptions
    df2 = session.read.option("COMPRESSION", "NONE").avro(
        Utils.escape_path(test_files.test_file_avro)
    )
    Utils.check_answer(
        df2,
...
def test_to_read_parquet_from_local(session, resources_path):

    test_files = TestFiles(resources_path)

    df = session.read.parquet(Utils.escape_path(test_files.test_file_parquet))
    Utils.check_answer(
        df,
        [
            Row(str="str1", num=1),
    ...
    # assert user cannot input a schema to read avro
    with pytest.raises(ValueError):
        session.read.schema(user_schema).parquet(
            Utils.escape_path(test_files.test_file_parquet)
        )

    # user can input customized formatTypeOptions
    df2 = session.read.option("COMPRESSION", "NONE").parquet(
        Utils.escape_path(test_files.test_file_parquet)
    )
    Utils.check_answer(
        df2,
