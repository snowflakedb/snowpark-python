...
def test_query_tags_in_session(session):
    ...
    try:
        session.query_tag = query_tag
        session.create_data_frame(["a", "b", "c"]).collect()
        session.create_data_frame(["a", "b", "c"]).count()
        session.create_data_frame(["a", "b", "c"]).show()
        session.create_data_frame(["a", "b", "c"]).first()
        session.create_data_frame(["a", "b", "c"]).to_pandas()
        session.create_data_frame(["a", "b", "c"]).create_or_replace_temp_view(
            view_name
        )
        session.create_data_frame(["a", "b", "c"]).create_or_replace_view(
            temp_view_name
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        """
    view_name = Utils.random_name()
    session.create_data_frame(["a", "b", "c"]).create_or_replace_view(view_name)
    Utils.drop_view(session, view_name)
    """,
        """
    temp_view_name = Utils.random_name()
    session.create_data_frame(["a", "b", "c"]).create_or_replace_temp_view(temp_view_name)
    Utils.drop_view(session, temp_view_name)
    """,
    ],
)
