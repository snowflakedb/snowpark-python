...
class TestDataFrame:
    ...
    def test_to_view_basic(self, session, native_pandas_df_basic) -> None:
        ...
        try:
            ...
            assert (
                ...
                in pd.to_view(obj=snow_dataframe, name=view_name)[0]["status"]
            )
    ...
    @sql_count_checker(query_count=6)
    def test_to_view_multiindex(self, session):
        try:
            # create table
            table_name = Utils.random_table_name()
            session.create_dataframe(
...
class TestSeries:
    @sql_count_checker(query_count=2)
    def test_to_view_basic(self, session, native_pandas_ser_basic) -> None:
        view_name = Utils.random_view_name()
        try:
            ...
            assert (
                "successfully created"
                in snow_series.to_view(name=view_name)[0]["status"]
            )
        finally:
            Utils.drop_view(session, view_name)
