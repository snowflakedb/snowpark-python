...
def test_read_snowflake_basic_query_with_weird_formatting(session, relaxed_ordering):
    ...
    with SqlCounter(query_count=expected_query_count):
        ...
        df = pd.read_snowflake(
            f"(((SELECT * FROM {table_name})))", relaxed_ordering=relaxed_ordering
        )
    ...
    [
        pytest.param(
            ...
            marks=pytest.mark.skip(
                ...
                "raise a SQL compilation error when relaxed_ordering=True"
            ),
        ),
        False,
    ],
...
def test_read_snowflake_query_table_not_exist_negative(relaxed_ordering) -> None:
...
@pytest.mark.parametrize("relaxed_ordering", [True, False])
def test_read_snowflake_query_table_bad_sql_negative(bad_sql, relaxed_ordering) -> None:
    expected_exception_type = (
        SnowparkPandasException if not relaxed_ordering else SnowparkSQLException
    )
    with pytest.raises(expected_exception_type) as ex:
        pd.read_snowflake(bad_sql, relaxed_ordering=relaxed_ordering)
