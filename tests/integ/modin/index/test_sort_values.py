...
# Tuple of (native index, whether indexer returned by Snowpark pandas will match native pandas)
SINGLE_TYPE_NATIVE_INDEX_TEST_DATA_WITH_INDEXER_MATCH = [
    (
        native_pd.Index(
            [1, 2, 3, 2, 3, 5, 6, 7, 8, 4, 4, 5, 6, 7, 1, 2, 1, 2, 3, 4, 3, 4, 5, 6, 7]
        ),
    ...
    (native_pd.Index([1.1, 2.2, 1.0, 1, 1.1, 2.2, 1, 1, 1, 2, 2, 2, 2.2]), True),
    (
        native_pd.Index(
            [1.1, np.nan, 2.2, 1.0, 1, 1.1, 2.2, 1, np.nan, 1, 1, 2, 2, 2, np.nan, 2.2]
        ),
    ...
    (native_pd.Index([1, 3, 1, 1, 1, 3, 1, 1, 1, 2, 2, 2, 3]), True),
    (
        native_pd.Index(
            [True, False, True, False, True, False, True, False, True, True], dtype=bool
        ),
...
@pytest.mark.parametrize(
    "native_index, match_pandas", SINGLE_TYPE_NATIVE_INDEX_TEST_DATA_WITH_INDEXER_MATCH
)
@pytest.mark.parametrize("ascending", [True, False])
@pytest.mark.parametrize("na_position", ["first", "last"])
@sql_count_checker(query_count=2)
def test_index_sort_values_return_indexer(
    native_index, match_pandas, ascending, na_position
):
    ...
    # In case the Snowpark pandas result does not match pandas, verify that it matches `np.argsort(kind=stable)`.
    if match_pandas:
        # The indexer generated by Snowpark pandas matches native pandas.
        assert np.array_equal(snow_res[1], native_res[1])
    else:
        # The indexer generated by Snowpark pandas does not match native pandas.
        # Compare with `np.argsort(kind=stable)`.
        # When `ascending` is False, the Index is sorted in descending order. There is no such direct option for
        # `np.argsort`. Therefore, it is improvised by negating all the values in the Index before sorting.
        if not ascending:
            array = (
                -native_index
                if is_numeric_dtype(native_index.dtype)
                # In case the type isn't numeric (it is string), turn them into ordinals.
                else [-ord(c) for c in native_index]
            )
        else:
            array = native_index
