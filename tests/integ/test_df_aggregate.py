...
@pytest.mark.parametrize("col1,col2", [("価格", "数量"), ("ราคา", "ปริมาณ"), ("😀", " 😂")])
def test_df_agg_with_nonascii_column_names(session, col1, col2):
    ...
    assert df.agg(count(col1), sum_(col2)).columns == [
        f'"COUNT(""{col1}"")"',
        f'"SUM(""{col2}"")"',
    ]
