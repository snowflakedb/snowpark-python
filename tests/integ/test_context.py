...
def test_context_configure_development_features(session):
    ...
    try:
        ...
        with mock.patch.object(context, "get_active_session", side_effect=RuntimeError("test")):
            context.configure_development_features(enable_trace_sql_errors_to_dataframe=True)
            assert context._enable_trace_sql_errors_to_dataframe is False
            assert context._enable_dataframe_trace_on_error is False

        # Test when get_active_session() returns a valid session
        with mock.patch.object(context, "get_active_session", return_value=mock.MagicMock()):
            context.configure_development_features(enable_trace_sql_errors_to_dataframe=True)
