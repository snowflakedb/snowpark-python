...
def test_sql_simplifier(session):
    ...
    with SqlCounter(query_count=0, describe_count=0):
        ...
        assert (
            count_number_of_ctes(
                Utils.normalize_sql(df4.queries["queries"][-1], session._new_line_token)
            )
            == 1
        )
        assert (
    ...
    with SqlCounter(query_count=0, describe_count=0):
        ...
        assert (
            count_number_of_ctes(
                Utils.normalize_sql(df5.queries["queries"][-1], session._new_line_token)
            )
            == 1
        )
        assert (
    ...
    with SqlCounter(query_count=0, describe_count=0):
        ...
        assert (
            count_number_of_ctes(
                Utils.normalize_sql(df6.queries["queries"][-1], session._new_line_token)
            )
            == 2
        )
        assert (
    ...
    with SqlCounter(query_count=0, describe_count=0):
        ...
        assert (
            count_number_of_ctes(
                Utils.normalize_sql(df9.queries["queries"][-1], session._new_line_token)
            )
            == 1
        )
        assert (
            Utils.normalize_sql(
                df9.queries["queries"][-1], session._new_line_token
            ).count(filter_clause)
            == 3
        )
