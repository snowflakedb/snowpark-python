...
def test_collect_stacktrace_in_query_tag(session):
    ...
    try:
        ...
        assert Utils.strip_tabs_and_new_lines(history.queries[2].sql_text).startswith(
            'SELECT "A", "B" FROM'
        )
        ...
        assert Utils.strip_tabs_and_new_lines(history.queries[4].sql_text).startswith(
            'SELECT "A", "B" FROM'
        )
