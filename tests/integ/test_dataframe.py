...
from snowflake.snowpark.functions import col, concat, lit, random, seq1, table_function, udtf, uniform, when
...
def test_generator_table_function(session):
    ...
    df = session.generator(seq1(1), uniform(1, 10, 2), rowcount=132).limit(3, offset=127)
    ...
    df = session.generator(seq1(1), uniform(1, 10, 2), timelimit=1, rowcount=150).limit(3, offset=127)
    Utils.check_answer(df, expected_result)
    ...
    # aliasing works
    df = session.generator(seq1(1).as_("pixel"), uniform(1, 10, 2).as_("unicorn"), rowcount=150).limit(3, offset=127)
    expected_result = [Row(pixel=127, unicorn=3), Row(pixel=-128, unicorn=3), Row(pixel=-127, unicorn=3)]
    Utils.check_answer(df, expected_result)
...
def test_generator_table_function_negative(session):
    ...
    with pytest.raises(ValueError) as ex_info:
        df = session.generator(rowcount=10)
