...
from snowflake.snowpark.functions import col, lit, sum as sum_, avg, count, when, sqrt
...
df3 = df2.filter(sqrt(col("NAME")) >= 1)  # Let's trigger sqrt-on-string error here, the SQL error would be something like Numeric value 'Alice' is not recognized.
