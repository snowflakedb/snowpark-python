...
- `DataFrameWriter.save_as_table` and `Session.write_pandas` again emit the deprecated `create_temp_table` field in their AST alongside `table_type`, so IR consumers built against the pre-deprecation contract keep working.
