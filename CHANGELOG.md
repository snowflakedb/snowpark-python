...
- Fixed a flaky `session.generator` integration test that required unique `SEQ*` values after `ORDER BY`. Snowflake sequence functions may emit duplicates under parallel generation.
