...
### Breaking Changes:
- Allowed using an empty list on UDF-level imports/packages (the argument `imports`/`packages` of `functions.udf()` and `functions.sproc()`) to indicate no import/package is used for this UDF or stored procedure. Previously an empty list means using session-level imports/packages.
