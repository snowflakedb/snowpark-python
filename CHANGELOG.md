...
- Added support for new functions in `snowflake.snowpark.functions`:
  ...
  - `array_max`
  - `explode_outer`
- Added support for pure Python packages specified via `Session.add_requirements` or `Session.add_packages`, now usable in stored procedures and UDFs even if packages are not present on the Snowflake Anaconda channel. Added Session parameter `custom_packages_upload_enabled` and `custom_packages_force_upload_enabled` to enable the feature mentioned above (both parameters default to `False`).
