...
from snowflake.snowpark.functions import avg, monthname
...
def calc_month_facts(df: DataFrame) -> DataFrame:
    ...
    """
    return (
        df.with_column("month", monthname("STARTTIME"))
        .group_by("month")
...
def calc_bike_facts(df: DataFrame) -> DataFrame:
    ...
    """
    return df.group_by("BIKEID").agg(
        (col("*"), "count"), avg("TRIPDURATION"), avg("RIDER_AGE")
    )
