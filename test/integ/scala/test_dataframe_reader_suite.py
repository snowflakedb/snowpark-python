...
def test_read_csv(session_cnx, mode):
    with session_cnx() as session:
...
@pytest.mark.parametrize("mode", ["select", "copy"])
def test_read_csv_incorrect_schema(session_cnx, mode):
    with session_cnx() as session:
        reader = get_reader(session, mode)
        test_file_on_stage = f"@{tmp_stage_name1}/{test_file_csv}"
        incorrect_schema = StructType(
...
def test_read_csv_with_more_operations(session_cnx):
    with session_cnx() as session:
        test_file_on_stage = f"@{tmp_stage_name1}/{test_file_csv}"
        df1 = (
            session.read()
...
@pytest.mark.parametrize("mode", ["select", "copy"])
def test_read_csv_with_format_type_options(session_cnx, mode):
    with session_cnx() as session:
...
@pytest.mark.parametrize("mode", ["select", "copy"])
def test_read_csv_with_special_chars_in_format_type_options(session_cnx, mode):
...
@pytest.mark.parametrize("mode", ["select", "copy"])
def test_read_json_with_no_schema(session_cnx, mode):
...
@pytest.mark.parametrize("mode", ["select", "copy"])
def test_read_avro_with_no_schema(session_cnx, mode):
...
@pytest.mark.parametrize("mode", ["select", "copy"])
def test_read_parquet_with_no_schema(session_cnx, mode):
...
@pytest.mark.parametrize("mode", ["select", "copy"])
def test_read_orc_with_no_schema(session_cnx, mode):
...
@pytest.mark.parametrize("mode", ["select", "copy"])
def test_read_xml_with_no_schema(session_cnx, mode):
...
def test_copy_option_force(session_cnx):
    test_file_on_stage = f"@{tmp_stage_name1}/{test_file_csv}"
...
def test_select_and_copy_on_non_csv_format_have_same_result_schema(session_cnx):
    path = f"@{tmp_stage_name1}/{test_file_parquet}"
...
@pytest.mark.parametrize("mode", ["select", "copy"])
def test_pattern(session_cnx, mode):
    with session_cnx() as session:
        assert (
            get_reader(session, mode)
            .schema(user_schema)
