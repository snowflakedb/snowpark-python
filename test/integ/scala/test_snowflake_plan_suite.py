...
def test_multiple_queries(session_cnx, db_parameters):
    with session_cnx(db_parameters) as session:
        ...
        queries = [
            Query(f"create or replace temporary table {table_name1} as select * from values(1::INT, 'a'::STRING),(2::INT, 'b'::STRING) as T(A,B)"),
            Query(f"select * from {table_name1}")]
        attrs = [Attribute("A", IntegerType()), Attribute("B", IntegerType())]
        pkg = AnalyzerPackage()

        try:
            plan = SnowflakePlan(queries, pkg.schema_value_statement(attrs), None, None, session)
        ...
        queries2 = [
            Query(f"create or replace temporary table {table_name2} as select * from values(3::INT),(4::INT) as T(A)"),
            Query(f"select * from {table_name2}")]
        attrs2 = [Attribute("C", LongType())]

        try:
            plan2 = SnowflakePlan(queries2, pkg.schema_value_statement(attrs2), None, None, session)
