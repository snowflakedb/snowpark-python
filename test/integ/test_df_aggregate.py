...
def test_df_agg_tuples_basic(session_cnx, db_parameters):
    with session_cnx(db_parameters) as session:
        ...
        res = df.agg([('first', 'min'), ('first', 'count'), ('first', 'max'), ('first', 'avg'), ('first', 'std')]).collect()
        ...
        # combine those together
        res = df.agg([('second', 'min'), ('second', 'count'), ('second', 'max'), ('second', 'avg'),
                      ('second', 'std')]).collect()
        assert res == [Row([4, 4, 6, 4.75, 0.9574272818339783])]

        # Get aggregations for both columns
        res = df.agg([('first', 'min'), ('second', 'count'), ('first', 'max'), ('second', 'avg'),
                      ('first', 'std')]).collect()
