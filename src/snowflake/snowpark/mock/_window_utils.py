...
class EntireWindowIndexer:
    def get_window_bounds(self, num_values, min_periods, center, closed, step):
        try:
            ...
            # Ensure this class inherits from BaseIndexer if pandas is available
            if not hasattr(self.__class__, "_base_class_set"):
                self.__class__.__bases__ = (BaseIndexer,)
                self.__class__._base_class_set = True
        except ImportError:
            import numpy as np

            # Fallback to object if pandas is not available
            pass
