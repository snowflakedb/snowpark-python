...
DUMMY_TIMEDELTA_INPUT = native_pd.to_timedelta(
    ["1day", "2h", "3m", "4s", "5ms", "6us", "7ns", "23 days 06:05:01.00003"]
    + ["-1day", "-2h", "-3m", "-4s", "-5ms", "-6us", "-7ns", "-999 days 06:05:01.00003"]
    + [native_pd.Timedelta.min, native_pd.Timedelta.max]  # fringe cases
)


def infer_return_type_using_dummy_data(
    func: Callable, input_type: DataType
) -> Optional[DataType]:
    """
    ...
    def merge_types(t1: DataType, t2: DataType) -> DataType:
        """
        Merge two types into as per the following rules:
        - Null + T = T
        - T + Null = T
    ...
    for x in input_data:
        try:
            inferred_type = merge_types(inferred_type, infer_object_type(func(x)))
        except Exception:
            pass

    if isinstance(inferred_type, TimedeltaType):
        # SNOW-1619940: pd.Timedelta is encoded as string.
        return StringType()
    return inferred_type


def deduce_return_type_from_function(
    func: Union[AggFuncType, UserDefinedFunction], input_type: Optional[DataType] = None
) -> Optional[DataType]:
    """
    ...
    if isinstance(func, list):
        ...
        return infer_return_type_using_dummy_data(func, input_type)
