...
class BaseSnowparkPandasDataset(BasePandasDataset):
    ...
    def aggregate(
        ...
        if axis == 1 and isinstance(self, pd.SeriesImpl):
        ...
        if need_reduce_dimension:
            if self._is_dataframe:
                result = pd.SeriesImpl(query_compiler=result._query_compiler)

            if isinstance(result, pd.SeriesImpl):
