...
class Row(tuple):
    ...
    def __getattr__(self, item):
        if self.__fields__:
            # So there are duplicates. Usually this doesn't happen.
            if len(set(self.__fields__)) != len(
                self.__fields__
            ):  # duplicate fields found
                try:
    ...
    def __copy__(self):
        return _restore_row_from_pickle(self, self._named_values, self.__fields__)
