...
class DataFrame:
    ...
    **Performing an action on a DataFrame**
    ...
    def dropna(
        self,
        min_non_nulls_per_row: Optional[int] = None,
        cols: Optional[List[str]] = None,
    ) -> "DataFrame":
        """
        Returns a new DataFrame that excludes all rows containing fewer than
        ``minNonNullsPerRow`` non-null and non-NaN values in the specified
        columns.

        Args:
            min_non_nulls_per_row: The minimum number of non-null and non-NaN
                ...
                row to be included. In each case:
                    ...
                    * If ``minNonNullsPerRow`` is less than 1, the method returns
                      the original DataFrame.

            cols: A list of the names of columns to check for null and NaN values.
                In each case:

                    * If ``cols`` is not provided or ``None``, all columns will be included.
        ...
        # if cols is not provided, dropna will be applied to all columns
        if cols is None:
            cols = self.columns
        # if min_non_nulls_per_row is not provided, drop a row if it contains any nulls
        if min_non_nulls_per_row is None:
            min_non_nulls_per_row = len(cols)

        # if min_non_nulls_per_row is less than 1, or no column is specified
        # to be dropped, return the dataframe directly
        if min_non_nulls_per_row < 1 or len(cols) == 0:
            return self
        # if min_non_nulls_per_row is greater than the number of columns,
        # drop a row only if all its values are null
        elif min_non_nulls_per_row > len(cols):
            return self.limit(0)
        else:
            df_col_type_dict = {
            ...
            normalized_col_name_set = {
                AnalyzerPackage.quote_name(col_name) for col_name in cols
            }
            col_counter = None
            for normalized_col_name in normalized_col_name_set:
                ...
                col = self.col(normalized_col_name)
                if isinstance(
                ...
                if col_counter:
                    ...
                    col_counter = is_na
            return self.where(col_counter >= min_non_nulls_per_row)

    def fillna(self, value_dict: Dict[str, Any]) -> "DataFrame":
        """
        Returns a new DataFrame that replaces all null and NaN values in the specified
        columns with the values provided.

        Args:
            value_dict: A ``dict`` that associates the names of columns with the
                values that should be used to replace null and NaN values in those
                columns. The value in ``value_dict`` cannot be ``tuple``, ``list``
                or ``dict``. If ``value_dict`` is empty, the method returns the
                original DataFrame.
        ...
        Examples::
            ...
            df.fillna({"a": 3.14, "b": 15})

        Note:
            If the type of a given value in ``replacement`` doesn't match the
        ...
        # return the dataframe directly if the dict is empty
        if not value_dict:
            return self
        ...
        for col_name, datatype in df_col_type_dict.items():
            ...
            if col_name in normalized_value_dict:
                ...
                if DataFrameNaFunctions._is_value_type_matching_for_na_function(
                    value, datatype
                ):
        ...
        return self.select(res_columns)

    def replace(
        self, replacement: Dict[Any, Any], cols: Optional[List[str]] = None
    ) -> "DataFrame":
        """
        Returns a new DataFrame that replaces values in the specified columns.

        Args:
            replacement: A ``dict`` that associates the original values with
                the replacement values. To replace a null value, use ``None``
                as the key in ``replacement``. To replace a NaN value, use
                ``float("nan")`` as the key in ``replacement``. If ``replacement``
                is empty, the method returns the original DataFrame.
            cols: A list of the names of columns in which the values should be
        ...
        Note:
            If the type of a given key or value in ``replacement`` doesn't match the
            column data type (e.g. a ``float`` for :class:`~snowflake.snowpark.types.StringType`
            column), this replacement will be skipped in this column. Especially,
        ...
        for col_name, datatype in df_col_type_dict.items():
            ...
            if col_name in normalized_col_name_set:
                ...
                for key, value in replacement.items():
                    if DataFrameNaFunctions._is_value_type_matching_for_na_function(
                        key, datatype
                    ) and DataFrameNaFunctions._is_value_type_matching_for_na_function(
                        value, datatype
                    ):
...
class DataFrameNaFunctions:
    ...
    def drop(
        ...
        """
        return self.df.dropna(min_non_nulls_per_row, cols)

    def fill(self, value_dict: Dict[str, Any]) -> "DataFrame":
        """
        Returns a new DataFrame that replaces all null and NaN values in the specified
        columns with the values provided.
        ...
        """
        return self.df.fillna(value_dict)
    ...
    def replace(
        ...
        """
        return self.df.replace(replacement, cols)
    ...
    def _is_value_type_matching_for_na_function(value: Any, datatype: DataType) -> bool:
        ...
        return (
            ...
            or (
                ...
                and isinstance(datatype, (FloatType, DoubleType))
            )
