...
{output_queries}"""
    ...
    def map(
        ...
        Args:
            func: A function to be applied to every element of the DataFrame.
            ...
            wrap_row: A flag to determine if the current row must be wrapped in an instance of the :class:`Row` class.
            extra_packages: packages to include in the `udtf` registration.
        ...
        Example 3::
            ...
            >>> new_df = df.map(lambda row: (row[1], row[0] * 3), output_types=[StringType(), IntegerType()], output_column_names=['col1', 'col2'])
            >>> new_df.show()
            -------------------
            |"COL1"  |"COL2"  |
            -------------------
        ...
        Example 4::
            >>> df = session.create_dataframe([10, 20, 30])
            >>> new_df = df.map(lambda x: x * x, output_types=[IntegerType()], wrap_row=False)
            >>> new_df.sort(new_df[0]).show()
            ---------
            |"C_1"  |
            ---------
            |100    |
        ...
        input_row_obj = Row(*self.columns) if wrap_row else None

        def wrap_result(result):
            if isinstance(result, list) or isinstance(result, Row):
                return tuple(result)
        ...
        map_udtf = self._session.udtf.register(
            ...
            is_permanent=False,
            packages=["snowflake-snowpark-python"] + extra_packages,
        )
        ...
        _input_cols = [self[column_name] for column_name in self.columns]
        return self.join_table_function(map_udtf(*_input_cols)).select(*_output_cols)
