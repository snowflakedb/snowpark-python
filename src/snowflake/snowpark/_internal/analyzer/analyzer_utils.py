...
def table_function_partition_spec(
    ...
    return f"""{OVER + LEFT_PARENTHESIS if over else EMPTY_STRING}{f"PARTITION BY {COMMA.join(partition_spec)}" if partition_spec else EMPTY_STRING}{SPACE}{f" ORDER BY {COMMA.join(order_spec)}" if order_spec else EMPTY_STRING}{RIGHT_PARENTHESIS if over else EMPTY_STRING}"""
