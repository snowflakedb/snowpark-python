...
class SnowflakeTable(LeafNode):
    def __init__(
        ...
        *,
        ...
        super().__init__()
        self.name = name
        # enable_auto_garbage_collection should only be set to True when it's a temp table
        # generated by Snowpark under the hood and users are not aware of it.
        if enable_auto_garbage_collection:
            session._inc_temp_table_ref_count(name)
            # it will be triggered when it gets garbage collected
