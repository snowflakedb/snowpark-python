...
class SnowflakeTable(LeafNode):
    def __init__(
        ...
        *,
        ...
        if is_temp_table_for_cleanup and session._temp_table_cleaner is not None:
            session._temp_table_cleaner.inc_ref_count(name)
            # the finalizer will be triggered when it gets garbage collected
            # and this table will be dropped finally
            self._finalizer = weakref.finalize(
                self, session._temp_table_cleaner.dec_ref_count, name
            )
