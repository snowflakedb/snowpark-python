...
def derive_dependent_columns_with_duplication(*expressions: "Optional[Expression]") -> list[str]:
    ...
    for exp in expressions:
        if exp is not None:
            child_dependency = exp.dependent_column_names_with_duplication()
            assert child_dependency is not None
            result.extend(child_dependency)
...
class ScalarSubquery(Expression):
    ...
    def dependent_column_names_with_duplication(self) -> Optional[list[str]]:
...
class Star(Expression):
    ...
    def dependent_column_names_with_duplication(self) -> list[str]:
        ...
        return (
            ...
            if self.expressions
            else [COLUMN_DEPENDENCY_ALL_STAR]
        )
...
class UnresolvedAttribute(Expression, NamedExpression):
    ...
    def dependent_column_names_with_duplication(self) -> list[str]:
        if self._dependent_column_names == COLUMN_DEPENDENCY_ALL:
            return [COLUMN_DEPENDENCY_ALL_STAR]
        else:
            return list(self._dependent_column_names)
...
class CaseWhen(Expression):
    ...
    def dependent_column_names_with_duplication(self) -> list[str]:
        ...
        if self.else_value is not None:
            exps.append(self.else_value)
        return derive_dependent_columns_with_duplication(*exps)
