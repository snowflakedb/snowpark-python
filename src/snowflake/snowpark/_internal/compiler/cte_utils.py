...
def find_duplicate_subtrees(
    ...
    def is_read_file_node(node: "TreeNode") -> bool:
        """
        Check if the current node is a ReadFileNode. Currently, we do not support
        deduplication for ReadFileNode due to SNOW-1911967.
        """
        if isinstance(node, SnowflakePlan) and (node.source_plan is not None):
            return isinstance(node.source_plan, ReadFileNode)
