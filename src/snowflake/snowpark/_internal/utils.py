...
def convert_ids_to_upper_case(name: str) -> str:
    """convert a fully qualfied name to upper case when needed. The double-quoted part won't be converted.
    This function doesn't validate whether the name has the right pattern.
    Instead, it assumes the name has the right pattern.
    """
    i = 0
    ...
    def find_next_id() -> str:
        ...
        iden = name[i]
        i += 1
        if iden == ".":  # the 2nd dot in format a..b
            return iden
        while i < len(name):
            if quoted:
                if i + 1 < len(name) and name[i + 1] == '"' and name[i] == '"':
                    iden += name[i : i + 2]
                    i += 2
                else:
                    iden += name[i]
                    if name[i] == '"':
                        i += 1  # skip the close quote
                        skip_space()
                        i += 1  # skip the first dot after quote
                        return iden
                    else:
                        i += 1
            else:
                if name[i] == ".":
                    i += 1
                    skip_space()
                    return iden.upper().rstrip()
                else:
                    iden += name[i]
                    i += 1
        if iden:
            return iden.strip().upper()
        return iden
    ...
    while next_id:
        if next_id != ".":
            next_id = "." + next_id  # in format `a..b`, next_id is the second dot.
        result = f"{result}{next_id}"
        next_id = find_next_id()
    return result
