...
class TypesPackage:
    ...
    @staticmethod
    def to_java_type(self, datatype):
        pass

    # TODO revisit when dealing with Java UDFs. We'll probably hard-code the return values.
    def to_udf_argument_type(self, datatype) -> str:
        pass
    ...
    def snow_type_to_sp_type(datatype: DataType) -> SPDataType:
        ...
        if type(datatype) == StructType:
            return SPStructType([
                SPStructField(field.name, TypesPackage.snow_type_to_sp_type(field.dataType),
                              field.nullable)
        ...
        if type(datatype) == ArrayType:
            return SPArrayType(TypesPackage.snow_type_to_sp_type(datatype.element_type),
                               contains_null=True)
        if type(datatype) == MapType:
            return SPMapType(TypesPackage.snow_type_to_sp_type(datatype.key_type),
                             TypesPackage.snow_type_to_sp_type(datatype.value_type),
    ...
    def to_sp_struct_type(struct_type: StructType) -> SPStructType:
        return TypesPackage.snow_type_to_sp_type(struct_type)
    ...
    def to_snow_struct_type(struct_type: SPStructType) -> StructType:
        return TypesPackage.sp_type_to_snow_type(struct_type)
