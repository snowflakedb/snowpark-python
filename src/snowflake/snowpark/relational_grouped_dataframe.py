...
class RelationalGroupedDataFrame:
    ...
    def _to_df(
        ...
        exclude_grouping_columns: bool = False,
        _ast_stmt: Optional[proto.Bind] = None,
    ...
    def agg(
        ...
        *exprs: Union[Column, Tuple[ColumnOrName, str], Dict[str, str]],
        ...
        Args:
            ...
            exclude_grouping_columns: If True, the resulting DataFrame will only contain
                the aggregated columns and exclude the grouping columns. Default is False.
    ...
    @publicapi
    def count(
        self, exclude_grouping_columns: bool = False, _emit_ast: bool = True
    ) -> DataFrame:
        """Return the number of rows for each group.

        Args:
            exclude_grouping_columns: If True, the resulting DataFrame will only contain
                the count column and exclude the grouping columns. Default is False.
        """
        df = self._to_df(
            [
                Alias(
